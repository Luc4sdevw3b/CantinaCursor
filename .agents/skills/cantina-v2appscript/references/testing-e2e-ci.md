...
31. fila da dona: **Reservas** → Recreio tarde → Ana Souza / 3º A / Coxinha → **Confirmar reserva**; produção `Coxinha • 1`; filtro Recreio manhã esconde a reserva; **Pesquisar reserva** `Ana` encontra e `ZZZZZZ` não; **Vincular aluno** `Ana Souza • ~8` → `vinculada a Ana Souza • ~8`; **Alterar reserva** turma `4º B`; **Entregar reserva** → `… • retirada`; disponibilidade `Coxinha • disponível 10 • reservado 0`; estoque físico `Coxinha • 10`. Sem abrir venda. Funcionário vê pesquisar/produção e não vê **Criar recreio**.
