...
A tela privada é uma só: hero com atalhos e uma área visível por vez. Só a área ativa é carregada; o botão **Atualizar** relê essa área. O login devolve `screen` (Vendas: produtos, vendas, estoque, reservas, dívidas, caixa) e `roster` (alunos, turmas, responsáveis e vínculos). Abrir Estoque, Agenda, Reservas, Caixa, Alunos ou Responsáveis reusa esse payload, sem nova ida ao Google. Pagamentos e crédito reusam o cadastro e só pedem a lista que falta (`listPayments` / `listCreditAccounts`).
