...
| ------------------------------------------------------------ | --------------------------------------------------: | -----------------------------------------------------: | ------------------------------------------------------------------------------------------------------------------------------------------------: | --------------------------------------------------------------: |
| Login + dashboard (área Vendas)                              |   ~4 s só em Vendas; login carregava todas as áreas |                                   1 round trip da tela |                                                                                                                              login + ~30–40 loads |                                           `loginE2E` = **1** (`screen` + `roster`) |
...
| Abrir aba Alunos                                             | 1 chamada, mas com leitura de cabeçalho em cada aba | 0 depois do login (`roster`); senão 1 chamada |                                                                                                                   **1** (`getStudentsScreenData`) |          **0** depois do login; **Atualizar** = `getStudentsScreenData` |
...
| Abrir Responsáveis depois do login                           | 1 chamada | 0 (`roster`) | **1** (`getFamilyScreenData`) | **0** |
| Abrir Pagamentos / Crédito depois do login                   | 1 chamada cheia cada | 1 lista barata cada | **1** (`getPaymentsScreenData` / `getCreditsScreenData`) | **1** (`listPayments` / `listCreditAccounts`) |
...
| Trocar turma do aluno                                        | 2 (`updateStudent` + `enrollStudent`) | 1 (`updateStudent` com `classroomId`) | **2** | **1** |
...
| Login + Vendas | `loginE2E` | **1** | uma ida (antes: 5,5 + 9,9 s em 2 chamadas) |
| Alunos | — | 0 | reusa `roster` do login |
| Responsáveis | — | 0 | reusa `roster` do login |
