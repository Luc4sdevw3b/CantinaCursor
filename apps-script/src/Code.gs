...
const RESERVATION_SLOTS_SHEET = '_reservation_slots';
const RESERVATION_SLOTS_HEADERS = [
  'id',
  'business_date',
  'label',
  'pickup_starts_at',
...
const RESERVATIONS_SHEET = '_reservations';
...
const RESERVATION_ITEMS_SHEET = '_reservation_items';
const RESERVATION_ITEMS_HEADERS = [
  'id',
  'reservation_id',
  'product_id',
  'description_snapshot',
...
const RESERVATION_STATUS_HISTORY_SHEET = '_reservation_status_history';
const RESERVATION_STATUS_HISTORY_HEADERS = [
  'id',
  'reservation_id',
  'from_status',
  'to_status',
...
const RESERVATIONS_MIGRATION_ID = '015_reservations';
const RESERVATIONS_MIGRATION_CHECKSUM =
  'id|business_date|label|pickup_starts_at|pickup_ends_at|cutoff_at|active|created_by|created_at|id|public_code|request_id|requester_name|student_name_text|classroom_text|contact_optional|slot_id|status|payment_status|linked_student_id|total_cents|created_at|updated_at|note|id|reservation_id|product_id|description_snapshot|quantity|unit_price_cents|line_total_cents|id|reservation_id|from_status|to_status|actor_id|created_at|reason';
const RESERVATION_STATUS_RESERVED = 'reserved';
const RESERVATION_STATUS_FULFILLED = 'fulfilled';
...
const SETTLEMENT_GUARDIAN_CREDIT = 'guardian_credit';
...
const PAYMENT_MODE_CREDIT_REMAINDER = 'credit_remainder';
const PAYMENT_MODE_ALL_CREDIT = 'all_credit';
...
function setupSchema() {
  ...
  const pending =
    ...
    applied.indexOf(RESERVATIONS_MIGRATION_ID) === -1;
  ...
  if (applied.indexOf(REVERSALS_MIGRATION_ID) === -1) {
    ...
    meta.appendRow(['schema_version', '14']);
    migrations.appendRow([
      REVERSALS_MIGRATION_ID,
      createdAt,
      CANTINA_APP_VERSION,
  ...
  if (applied.indexOf(RESERVATIONS_MIGRATION_ID) === -1) {
    getOrCreateSheet(
      spreadsheet,
      RESERVATION_SLOTS_SHEET,
      RESERVATION_SLOTS_HEADERS,
    );
    getOrCreateSheet(spreadsheet, RESERVATIONS_SHEET, RESERVATIONS_HEADERS);
    getOrCreateSheet(
      spreadsheet,
    ...
    migrations.appendRow([
      ...
      createdAt,
      CANTINA_APP_VERSION,
      RESERVATIONS_MIGRATION_CHECKSUM,
      'Cria recreios, reservas, itens e histórico de status',
    ]);
  }
  if (pendingCopy) {
  ...
  appendGuardianLinkRecord({
    ...
    can_use_guardian_credit:
      input.canUseGuardianCredit === undefined
        ? current
          ? current.can_use_guardian_credit
          : 'false'
    ...
    auto_settle_debt_from_guardian_credit:
      input.autoSettle === undefined
        ? current
          ? current.auto_settle_debt_from_guardian_credit
          : 'false'
        : input.autoSettle
          ? 'true'
          : 'false',
...
function linkGuardian(sessionToken, studentId, guardianId, payload) {
  ...
  return withScriptLock(function () {
    ...
    appendGuardianLinkGs({
      ...
      canUseGuardianCredit:
        payload && payload.canUseGuardianCredit === undefined
          ? undefined
          : payload && payload.canUseGuardianCredit === true,
      autoSettle:
        payload && payload.autoSettle === undefined
          ? undefined
          : payload && payload.autoSettle === true,
...
function availableForGs(dayId, productId, businessDate) {
  return (
    physicalForGs(dayId, productId) -
    reservedQuantityGs(productId, businessDate)
  );
}
...
function seedE2ESlotsUnlocked() {
  ...
  openNamedSheet(RESERVATION_SLOTS_SHEET, RESERVATION_SLOTS_HEADERS).appendRow([
    ...
    combineCivilTimeSaoPauloGs(today, '09:15'),
    'true',
    actor,
    now,
  ]);
  openNamedSheet(RESERVATION_SLOTS_SHEET, RESERVATION_SLOTS_HEADERS).appendRow([
    ...
    combineCivilTimeSaoPauloGs(today, '18:00'),
    'true',
    actor,
    now,
  ]);
...
function listPaymentCreditAllocationRecords() {
  return listSheetRecords(
    openNamedSheet(
      PAYMENT_CREDIT_ALLOCATIONS_SHEET,
      PAYMENT_CREDIT_ALLOCATIONS_HEADERS,
...
function planSettlementsGs(
  ...
  if (kind === PAYMENT_FIADO) {
    const personalBalance = Number.isInteger(creditBalanceCents)
      ? Math.max(0, creditBalanceCents)
      : 0;
    const guardianBalance = Number.isInteger(guardianCreditCents)
      ? Math.max(0, guardianCreditCents)
      : 0;
    const personalUsed = Math.min(personalBalance, net);
    const afterPersonal = net - personalUsed;
    const guardianUsed = Math.min(guardianBalance, afterPersonal);
    const fiadoCents = afterPersonal - guardianUsed;
    const rows = [];
    if (personalUsed > 0) {
      rows.push({
        kind: SETTLEMENT_CREDIT,
        amount_cents: String(personalUsed),
      });
    }
    if (guardianUsed > 0) {
      rows.push({
        kind: SETTLEMENT_GUARDIAN_CREDIT,
        amount_cents: String(guardianUsed),
      });
    }
    if (fiadoCents > 0) {
...
function saleSummaryLabelGs(
  ...
  if (guardianCreditLabel) {
    extras.push('crédito resp. ' + guardianCreditLabel);
  }
  if (changeLabel) {
...
function toSaleViewGs(sale) {
  ...
  const guardianCreditCents = settlementRows
    .filter(function (item) {
      return item.kind === SETTLEMENT_GUARDIAN_CREDIT;
    })
    .reduce(function (total, item) {
      return total + Number(item.amount_cents);
    }, 0);
  ...
  const guardianCreditLabel =
    guardianCreditCents > 0 ? formatBrlGs(guardianCreditCents) : null;
  return {
    id: sale.id,
...
function createSaleUnlocked(userId, payload, actorIsOwner) {
  ...
  const personalUsedCents = plannedSettlements.rows
    .filter(function (row) {
  ...
  const guardianUsedCents = plannedSettlements.rows
    .filter(function (row) {
      return row.kind === SETTLEMENT_GUARDIAN_CREDIT;
    })
    .reduce(function (total, row) {
      return total + Number(row.amount_cents);
    }, 0);
...
function parseFamilyPaymentModeGs(value) {
  if (
    value === PAYMENT_MODE_OLDEST_FIRST ||
    value === PAYMENT_MODE_SELECTED ||
    value === PAYMENT_MODE_MANUAL ||
  ...
  throw new Error(
    'PAYMENT_MODE_UNSUPPORTED: Use quitar um filho, selecionadas, alocação manual, dívida + crédito ou tudo crédito.',
  );
}
...
function leftoverUnexplainedGs() {
  throw new Error(
    'PAYMENT_LEFTOVER_UNEXPLAINED: A sobra precisa ir para o crédito do responsável.',
  );
}

function planFamilyManualAllocationsGs(
  amount,
  receivables,
  allocations,
  leftoverAllowed,
  ...
  if (leftoverAllowed) {
    if (total <= 0 || total >= amount) {
      leftoverUnexplainedGs();
    }
    return { allocations: rows, creditCents: amount - total };
  }
  if (total < amount) {
    leftoverUnexplainedGs();
  }
  ...
  return { allocations: rows, creditCents: 0 };
}

function planFamilyPaymentGs(
  amountCents,
  mode,
  receivables,
  studentId,
  ...
  const parsedMode = parseFamilyPaymentModeGs(mode);
  const amount = parsePaymentAmountGs(amountCents);
  if (parsedMode === PAYMENT_MODE_ALL_CREDIT) {
    return { allocations: [], creditCents: amount };
  }
  if (parsedMode === PAYMENT_MODE_CREDIT_REMAINDER) {
    if (allocations && allocations.length) {
      return planFamilyManualAllocationsGs(
        amount,
        receivables,
        allocations,
        true,
    ...
    const open = receivables.filter(function (item) {
      return item.remaining_cents > 0;
    });
    const total = open.reduce(function (sum, item) {
      return sum + item.remaining_cents;
    }, 0);
    if (!open.length || amount <= total) {
      leftoverUnexplainedGs();
    }
    return {
      allocations: allocateOldestFirstGs(total, open),
      creditCents: amount - total,
    };
  }
  if (parsedMode === PAYMENT_MODE_OLDEST_FIRST && !studentId) {
    throw new Error(
      'PAYMENT_FAMILY_CHILD_REQUIRED: Escolha o filho para quitar.',
    );
  }
  var pool = receivables;
  if (parsedMode === PAYMENT_MODE_OLDEST_FIRST) {
    pool = receivables.filter(function (item) {
      return item.charged_student_id === studentId;
    });
  }
  if (parsedMode === PAYMENT_MODE_MANUAL) {
    return planFamilyManualAllocationsGs(amount, pool, allocations, false);
  }
  if (parsedMode === PAYMENT_MODE_SELECTED) {
    const selected = selectedReceivableIds || [];
    if (!selected.length) {
  ...
  return {
    allocations: allocateOldestFirstGs(amount, pool),
    creditCents: 0,
  };
}
...
function familyPaymentSummaryLabelGs(
  guardianLabel,
  amountLabel,
  method,
  childLines,
  ...
  const methodLabel = method === PAYMENT_METHOD_CASH ? 'Dinheiro' : 'PIX';
  const parts = [guardianLabel, amountLabel, methodLabel];
  childLines.forEach(function (line) {
    parts.push(line.studentLabel + ' ' + line.amountLabel);
  });
  if (creditLabel) {
    parts.push('crédito ' + creditLabel);
  }
  return parts.join(' • ');
}
...
function familyPaymentChildLinesGs(paymentId) {
  ...
  return order.map(function (studentId) {
    return {
      ...
      amountLabel: formatBrlGs(sums[studentId]),
    };
  });
}
...
function toPaymentViewGs(payment) {
  ...
  if (payment.payer_guardian_id) {
    const guardianLabel = guardianLabelGs(payment.payer_guardian_id);
    const creditCents = paymentCreditCentsGs(payment.id);
    return {
      id: payment.id,
      payerStudentId: '',
      ...
      summaryLabel: familyPaymentSummaryLabelGs(
        guardianLabel,
        amountLabel,
        payment.method,
        familyPaymentChildLinesGs(payment.id),
        creditCents > 0 ? formatBrlGs(creditCents) : '',
      ),
...
function allocatableReceivablesForGuardianGs(guardianId) {
  const linked = {};
  listActiveGuardianLinksGs().forEach(function (link) {
  ...
  return listReceivableRecords()
    .filter(function (item) {
      return linked[item.charged_student_id];
    })
    .map(function (item) {
      return {
        id: item.id,
...
function createFamilyPaymentUnlocked(userId, payload) {
  const guardianId =
    payload && payload.guardianId ? String(payload.guardianId) : '';
  if (!guardianId) {
    throw new Error(
      'PAYMENT_GUARDIAN_REQUIRED: Escolha o responsável do pagamento.',
    );
  }
  const guardian = latestGuardianById(guardianId, true);
  const method = parsePaymentMethodGs(payload && payload.method);
  const studentId =
    payload && payload.studentId ? String(payload.studentId) : '';
  if (payload && payload.mode === PAYMENT_MODE_OLDEST_FIRST && studentId) {
    const linked = listActiveGuardianLinksGs().some(function (link) {
      return link.guardian_id === guardian.id && link.student_id === studentId;
    });
    if (!linked) {
      throw new Error(
        'PAYMENT_CHILD_NOT_LINKED: Este responsável não está vinculado a esse aluno.',
      );
    }
  }
  const planned = planFamilyPaymentGs(
    payload && payload.amountCents,
    payload && payload.mode,
    allocatableReceivablesForGuardianGs(guardian.id),
    studentId,
    payload && payload.selectedReceivableIds,
    payload && payload.allocations,
  );
  ...
  if (planned.creditCents > 0) {
    const account = ensureGuardianCreditAccountGs(guardian.id, userId, now);
...
function findGuardianCreditAccountGs(guardianId) {
  const accounts = listCreditAccountRecords().filter(function (account) {
    return (
...
function listActiveGuardianLinksGs() {
  const latest = {};
  listGuardianLinkRecords().forEach(function (record) {
    latest[record.id] = record;
  });
...
function usableGuardianCreditsGs(studentId) {
  return listActiveGuardianLinksGs()
    .filter(function (link) {
      return (
        link.student_id === studentId && link.can_use_guardian_credit === 'true'
      );
    })
    .sort(function (left, right) {
      return (
    ...
    .map(function (link) {
      const account = findGuardianCreditAccountGs(link.guardian_id);
      if (!account) {
        return null;
      }
      const balance = creditBalanceCentsGs(account.id);
      return balance > 0 ? { account: account, balance: balance } : null;
    })
    .filter(function (item) {
      return item;
    });
...
function ensureGuardianCreditAccountGs(guardianId, userId, now) {
  const existing = findGuardianCreditAccountGs(guardianId);
  if (existing) {
    return existing;
  }
  const accountId = Utilities.getUuid();
  ...
  const studentsSheet = openNamedSheet(
    CREDIT_ACCOUNT_STUDENTS_SHEET,
    CREDIT_ACCOUNT_STUDENTS_HEADERS,
  );
  listActiveGuardianLinksGs()
    .filter(function (link) {
      return link.guardian_id === guardianId;
    })
    .forEach(function (link) {
      studentsSheet.appendRow([
        accountId,
        link.student_id,
        link.can_use_guardian_credit === 'true' ? 'true' : 'false',
        'true',
      ]);
    });
  return {
...
function guardianLabelGs(guardianId) {
  const guardian = latestGuardianById(guardianId, true);
  const relation = guardian.relation_label
    ? ' • ' + guardian.relation_label
    : '';
  return guardian.full_name + relation;
}
...
function toCreditViewGs(account) {
  const ownerLabel =
    account.owner_type === CREDIT_OWNER_GUARDIAN
      ? guardianLabelGs(account.owner_guardian_id)
      : (function () {
  ...
  return {
    ...
    summaryLabel: creditSummaryLabelGs(ownerLabel, balanceLabel),
  };
}
...
function listCreditAccounts(sessionToken) {
  ...
  const guardians = records
    .filter(function (account) {
      return account.owner_type === CREDIT_OWNER_GUARDIAN;
    })
    .slice()
    .reverse()
    .map(function (account) {
  ...
  return personal.concat(guardians);
}
...
function depositGuardianCreditUnlocked(userId, payload) {
  ...
  if (!guardianId) {
    throw new Error(
      'CREDIT_GUARDIAN_REQUIRED: Escolha o responsável do crédito.',
    );
  }
  const guardian = latestGuardianById(guardianId, true);
  const method = parsePaymentMethodGs(payload && payload.method);
  ...
  listActiveGuardianLinksGs().forEach(function (link) {
    if (
      ...
      autoSettleIds[link.student_id] = true;
    }
  });
  const receivables = listReceivableRecords()
    .filter(function (item) {
      return autoSettleIds[item.charged_student_id];
    })
    .map(function (item) {
      return {
        id: item.id,
...
function refundGuardianCreditUnlocked(userId, payload) {
  const guardianId =
    payload && payload.guardianId ? String(payload.guardianId) : '';
  if (!guardianId) {
    throw new Error(
  ...
  const guardian = latestGuardianById(guardianId, true);
  const account = findGuardianCreditAccountGs(guardian.id);
  const balance = account ? creditBalanceCentsGs(account.id) : 0;
  var amount;
  try {
    amount = parseCentsGs(payload && payload.amountCents);
...
function combineCivilTimeSaoPauloGs(civilDate, clock) {
  if (!isCivilDate(civilDate) || !/^([01]\d|2[0-3]):([0-5]\d)$/.test(clock)) {
    throw new Error('INVALID_SLOT_TIME: Informe o horário no formato HH:mm.');
  }
  return new Date(civilDate + 'T' + clock + ':00.000-03:00').toISOString();
}

function formatSaoPauloClockGs(iso) {
  return Utilities.formatDate(new Date(iso), 'America/Sao_Paulo', 'HH:mm');
}

function listReservationSlotRecords() {
  return listSheetRecords(
    openNamedSheet(RESERVATION_SLOTS_SHEET, RESERVATION_SLOTS_HEADERS),
    RESERVATION_SLOTS_HEADERS,
  );
}

function listReservationRecords() {
  return listSheetRecords(
    openNamedSheet(RESERVATIONS_SHEET, RESERVATIONS_HEADERS),
    RESERVATIONS_HEADERS,
  );
}

function listReservationItemRecords() {
  return listSheetRecords(
    openNamedSheet(RESERVATION_ITEMS_SHEET, RESERVATION_ITEMS_HEADERS),
    RESERVATION_ITEMS_HEADERS,
  );
}

function listReservationHistoryRecords() {
  return listSheetRecords(
    openNamedSheet(
      RESERVATION_STATUS_HISTORY_SHEET,
      RESERVATION_STATUS_HISTORY_HEADERS,
...
function createPublicCodeGs() {
  let code = '';
  for (let index = 0; index < 6; index += 1) {
    code += PUBLIC_CODE_ALPHABET.charAt(
      Math.floor(Math.random() * PUBLIC_CODE_ALPHABET.length),
    );
  }
  return code;
}

function uniquePublicCodeGs() {
  const existing = latestRecordsById(listReservationRecords());
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const code = createPublicCodeGs();
    const taken = existing.filter(function (item) {
      return item.public_code === code;
    })[0];
    if (!taken) {
      return code;
    }
  }
  return createPublicCodeGs();
}

function reservationStatusLabelGs(status) {
...
function toReservationSlotGs(slot, nowIso) {
  const open =
    slot.active === 'true' && Date.parse(nowIso) < Date.parse(slot.cutoff_at);
  return {
    id: slot.id,
    ...
    openForReservations: open,
    summaryLabel:
      slot.label +
      ' • corte ' +
      formatSaoPauloClockGs(slot.cutoff_at) +
      ' • retirada ' +
      formatSaoPauloClockGs(slot.pickup_starts_at) +
      '–' +
      formatSaoPauloClockGs(slot.pickup_ends_at),
  };
}
...
function toReservationViewGs(reservation) {
  ...
  return {
    ...
    summaryLabel:
      reservation.student_name_text +
      ' • ' +
      reservation.classroom_text +
      ' • ' +
      names.join(', ') +
      ' • ' +
      formatBrlGs(Number(reservation.total_cents)) +
      ' • ' +
      slotLabel +
      ' • ' +
      reservationStatusLabelGs(reservation.status),
    items: items,
...
function toReservationsSetupGs() {
  ...
  const reservableProducts = latestRecordsById(listProductRecords())
    .filter(function (item) {
      return item.active === 'true' && item.reservable === 'true';
    })
    .map(function (item) {
      return {
        id: item.id,
...
function createReservationSlotUnlocked(userId, payload) {
  ...
  if (label.length < 2 || label.length > 40) {
    throw new Error('SLOT_LABEL_REQUIRED: Informe o nome do recreio.');
  }
  const businessDate = resolveBusinessDateGs(
    payload && payload.businessDate ? payload.businessDate : todayCivil(),
  );
  const cutoffAt = combineCivilTimeSaoPauloGs(
    businessDate,
    String((payload && payload.cutoffTime) || ''),
  );
  const pickupStartsAt = combineCivilTimeSaoPauloGs(
    businessDate,
    String((payload && payload.pickupStartTime) || ''),
  );
  const pickupEndsAt = combineCivilTimeSaoPauloGs(
    businessDate,
    String((payload && payload.pickupEndTime) || ''),
  );
  if (Date.parse(pickupEndsAt) <= Date.parse(pickupStartsAt)) {
    throw new Error(
      'SLOT_WINDOW_INVALID: O fim da retirada precisa ser depois do início.',
    );
  }
  if (Date.parse(cutoffAt) > Date.parse(pickupStartsAt)) {
    throw new Error(
...
function createReservationUnlocked(userId, payload) {
  if (payload && String(payload.status || '').toLowerCase() === 'prepared') {
    throw new Error(
      'RESERVATION_PREPARED_UNSUPPORTED: Não existe estado Preparada. A reserva fica reservada até a retirada, o cancelamento ou a não retirada.',
    );
  }
  ...
  const slotId = payload && payload.slotId ? String(payload.slotId) : '';
  if (!REQUEST_ID_PATTERN.test(slotId)) {
    throw new Error(
      'INVALID_ID: ID deve ser UUID imutável, nunca número da linha.',
    );
  }
  ...
  if (Date.parse(new Date().toISOString()) >= Date.parse(slot.cutoff_at)) {
    throw new Error(
      'RESERVATION_CUTOFF_PASSED: O horário de corte deste recreio já passou.',
    );
  }
  ...
  if (!items.length) {
    throw new Error(
      'RESERVATION_ITEMS_REQUIRED: Informe ao menos um item para reservar.',
    );
  }
  const products = latestRecordsById(listProductRecords());
  const parsed = [];
  ...
  items.forEach(function (raw) {
    const productId = raw && raw.productId ? String(raw.productId) : '';
    ...
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20) {
      throw new Error(
        'RESERVATION_QUANTITY_INVALID: A quantidade reservada precisa ser um inteiro entre 1 e 20.',
      );
    }
    const product = products.filter(function (item) {
    ...
    if (product.active !== 'true') {
      throw new Error(
        'PRODUCT_INACTIVE: Produto inativo não entra em reserva nova.',
      );
    }
    if (product.reservable !== 'true') {
      throw new Error(
        'PRODUCT_NOT_RESERVABLE: Este produto não aceita reserva.',
      );
    }
    needed[productId] = (needed[productId] || 0) + quantity;
    parsed.push({
  ...
  const day = requireInventoryDayGs(slot.business_date);
  Object.keys(needed).forEach(function (productId) {
    const product = products.filter(function (item) {
      return item.id === productId;
    })[0];
    ...
    if (
      availableForGs(day.id, productId, slot.business_date) < needed[productId]
    ) {
      throw new Error(
        'RESERVATION_UNAVAILABLE: Não há disponibilidade suficiente para esta reserva.',
      );
    }
  });
  const now = new Date().toISOString();
  const reservationId = Utilities.getUuid();
  const totalCents = parsed.reduce(function (total, item) {
  ...
  openNamedSheet(RESERVATIONS_SHEET, RESERVATIONS_HEADERS).appendRow([
    ...
    studentName,
    classroom,
    String((payload && payload.contactOptional) || ''),
    slot.id,
